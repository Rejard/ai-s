...
### 6. 35대 시스템 무결성 자가 진단 확장 및 테스트 가드 도입

최근 운영 안정성을 고도화하기 위해 시스템 무결성 검증 프로세스와 회귀 테스트 스위트를 대폭 확장하였습니다.

#### (1) 무결성 진단 노드 확장 (25개 ➔ 35개)
기존 25개 진단 노드 체계에서 10개의 신규 노드를 추가하여 총 35개 노드로 정교화했습니다.
...
- **PM2 경고 기준 현실화**: 누적 프로세스 재시작 경고 임계값을 기존 50회에서 100회로 상향하여 알림 오탐률을 완화했습니다.

#### (2) 프론트엔드 UI 대시보드 시각화 연동
- PC 및 모바일 관리자 대시보드(`admin_pc_dashboard.jsx`, `admin_mobile_dashboard.jsx`) 상에서 진단 노드 개수(35개) 및 카테고리 범위 인덱스를 완전히 동기화했습니다.
- 카테고리별(핵심 알고리즘: 9, 외부 인프라: 5, 보안: 5, 의회 하위 작업: 11, Shadow Racing: 5) 상세 리스트 필터링 및 실시간 ERROR/WARN 배지 카운트 렌더링을 적용했습니다.

#### (3) 신규 단위 및 통합 테스트 스위트 4종 도입
...
4. **systemDiagnostics.test.js** ([systemDiagnostics.test.js](file:///c:/home/ai-s/backend/systemDiagnostics.test.js))
   - 확장된 35개 진단 노드가 규격화된 스키마에 따라 정상 배치되어 동작하는지, 전체 카테고리 경계가 정확히 일치하는지, 엔진 모드 정합성 불일치나 API Key 유실 시 ERROR/WARNING 상태 판정이 올바르게 내려지는지 종합 검증합니다.
