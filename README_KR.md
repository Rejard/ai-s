...
## 📲 회원 온보딩 및 지갑 연동 프로세스 (User Onboarding & Wallet Integration)

최근 AI-S 플랫폼은 회원들의 편리한 가입 및 자산 가시성 극대화를 위해 회원 온보딩 절차를 획기적으로 개선하고 초간소화 개편을 완료하였습니다.

### 1. 구글 OAuth 간편 로그인 기반 가입 신청
* **신속 가입**: 복잡한 Web3 지갑 설치 없이, 전 세계 표준 보안 프로토콜인 구글 간편 연동 로그인만으로 즉시 가입 프로세스가 가동됩니다.
* **가상 지갑 할당**: 가입 단계에서는 실제 지갑 주소가 필요하지 않으며, 시스템 내부 알고리즘에 의해 이메일 해시 기반 고유 가상 지갑(0x+MD5 Email Padding 주소)이 자동으로 매칭됩니다.
* **추천인(매니저) 검증 및 KYC**: 자금 세탁 및 부정 가입 방지를 위해 정당한 추천 매니저의 지갑 주소 검증과 신분증 사본을 안전하게 암호화하여 제출합니다.

### 2. 본사 24시간 수동 대조 심사 및 승인
* **수동 심사 체계**: 제출된 KYC 서류와 지정 매니저는 본사 배심원단 전담 매니저들에 의해 수동으로 철저히 판별 및 심사됩니다.
* **24시간 자동 만료 가드**: 24시간 이내에 본사 승인이 완료되지 않는 미승인 신청 건은 보안을 위해 신분증 이미지 및 신청 데이터가 서버에서 완전히 물리적으로 영구 파기 소거 처리됩니다.

### 3. 트러스트월넷 디앱 브라우저 기반 동기화 및 1회성 위임 승인
* **온체인 주소 자동 동기화**: 본사 가입 승인이 성료되면 대시보드에 접근할 수 있게 되며, 트러스트월넷(Trust Wallet) 디앱 브라우저로 대시보드에 접속 시 실제 동작 중인 온체인 활성 지갑 주소가 안전하게 자동 동기화되어 표출됩니다.
* **개인 지갑 주소 전체 노출**: 내 개인 지갑 주소(SUT 입출금용)는 일부 자리 가림(Hiding) 처리 없이 전체 42자리 주소가 모바일에 최적화되어 완전 노출되므로, 모바일 폰에서도 가로 잘림이나 줄바꿈 누락 없이 안전하게 텍스트를 식별하고 손쉽게 복증(복사)할 수 있습니다.
* **SUT 자산 거래 위임 승인 확인 (최초 1회 필수)**:
  * 대시보드 및 설정 탭 내에 불안한 적색 위협 경고나 불필요한 사족 설명 가이드 카드를 전면 배제했습니다.
  * 실시간 온체인 승인 체크 엔진을 통해 `🟢 승인 완료 (거래가능)` 및 `⚠️ 미승인 (위임 필요)`과 같은 정교하고 차분한 다크 그레이형 알림 배지 카드로 교체되어 일관성 있는 미니멀하고 프리미엄급인 사용자 디자인을 선사합니다.
